...
class PiApplication:
    ...
    def __init__(self, config, plugin_manager):
        ...
        if not isinstance(init_size, str):
            self._window = PiWindow(title, init_size, color=init_color, text_color=init_text_color, debug=init_debug)
        else:
    ...
    def _initialize(self):
        ...
        for chx in choices:
            if chx not in [1, 2, 3, 4]:
                LOGGER.warning("Invalid captures number '%s' in config, fallback to '%s'", chx, self.capture_choices)
        ...
        if all(self.buttons.value):
            ...
            if self._multipress_timer.is_timeout():
                ...
                if self._menu and self._menu.is_shown():
                    ...
                    event = pygame.event.Event(BUTTONDOWN, capture=1, printer=1, button=self.buttons)
            ...
            if self._menu and self._menu.is_shown():
                ...
                event = pygame.event.Event(BUTTONDOWN, capture=1, printer=0, button=self.buttons.capture)
    ...
    def _on_button_printer_held(self):
        ...
        if all(self.buttons.value):
            ...
            if self._menu and self._menu.is_shown():
                ...
                event = pygame.event.Event(BUTTONDOWN, capture=0, printer=1, button=self.buttons.printer)
...
def main():
    ...
    parser.add_argument(
        "--version", action="version", version=pibooth.__version__, help="show program's version number and exit"
    )

    parser.add_argument("--config", action="store_true", help="edit the current configuration and exit")

    parser.add_argument("--translate", action="store_true", help="edit the GUI translations and exit")

    parser.add_argument("--reset", action="store_true", help="restore the default configuration/translations and exit")
