...
class RpiCamera(BaseCamera):
    ...
    def _show_overlay(self, text, alpha):
        ...
        if self._window:  # No window means no preview displayed
            ...
            self._overlay = self._cam.add_overlay(
                image.tobytes(), image.size, layer=3, window=tuple(rect), fullscreen=False
            )
    ...
    def preview(self, window, flip=True):
        ...
        if self._cam.hflip:
            if flip:
                ...
                flip = True
        self._cam.start_preview(resolution=(rect.width, rect.height), hflip=flip, fullscreen=False, window=tuple(rect))
