...
def write_log(text: str, new_section: bool = False) -> None:
    ...
    global LOGFILE
    if not LOGFILE:
        LOGFILE = open(APPNAME + ".log", "w")  # noqa: SIM115 -- global log file closed at exit
...
def print_config(config: Any, parent: str = "") -> None:
    ...
    gp_widget_types = {
        gp.GP_WIDGET_WINDOW: "Window toplevel",
    ...
    for child in config.get_children():
        ...
        if child.get_type() == gp.GP_WIDGET_SECTION:
            ...
            if child.get_type() == gp.GP_WIDGET_RADIO:
                write_log(f"  Choices     : {list(child.get_choices())}")
...
def main() -> None:
    ...
    preview_compat = abilities.operations & gp.GP_OPERATION_CAPTURE_PREVIEW == gp.GP_OPERATION_CAPTURE_PREVIEW
    write_log(f"* Preview compatible: {preview_compat}")
    capture_compat = abilities.operations & gp.GP_OPERATION_CAPTURE_IMAGE == gp.GP_OPERATION_CAPTURE_IMAGE
